...
_PRODUCT = r"\b(?:ads?|adverts?|advertisements?|commercials?|spotify(?:'s)?|app|playback|audio|music|sound|volume)\b"
_HARM_TARGET = r"(?:ears?|eardrums?|hearing|speakers?|headphones?|earbuds?|phone|computer|car|hands?|fingers?|eyes?)"
_HARM = (
    r"(?:\b(?:blow(?:s|ing)?(?: out)?|blew(?: out)?|blown|damage[ds]?|damaging|ruin(?:ed|ing)?|"
    r"broke[n]?|destroy(?:ed|ing)?|hurt(?:s|ing)?|injur(?:ed|ing)|burn(?:ed|ing)|burst|ruptur(?:ed|ing))\s+"
    r"(?:to\s+)?(?:(?:my|our|the|his|her|their|your|car|new|both)\s+){0,3}" + _HARM_TARGET
    + r"\b|\b(?:go(?:ing)?|gone|become|becoming|made (?:me|us)) (?:partially |permanently )?deaf\b|"
    r"\b(?:lose|losing|lost) (?:my |our |their |your )?hearing\b|\b(?:hearing (?:damage|loss)|"
    r"(?:my|our) ears? (?:are |is |were |was |started |start )?(?:ringing|bleeding|hurting))\b)"
)
_PRODUCT_HARM_PATTERNS = (
    _PRODUCT + r"[^.!?;\n]{0,120}?(?P<harm>" + _HARM + r")",
    r"(?P<harm>" + _HARM + r")[^.!?;\n]{0,30}\b(?:because of|due to|from|after|caused by)\b[^.!?;\n]{0,45}" + _PRODUCT,
    _PRODUCT + r"[^.!?;\n]{0,80}\b(?:quieter|too loud|deafening|blaring|blasting|excessive volume)\b"
    r"[.!?;\s]*[^.!?;\n]{0,75}?(?P<harm>" + _HARM + r")",
)
_ACCOUNT_OBJECT = r"(?:(?:my|our|the|this|its|your)\s+)(?:spotify\s+)?(?:account|login(?: email)?|email|password|username|identity)\b"
_PATTERNS: dict[str, tuple[str, ...]] = {
    ...
    "security": (
        ...
        r"\b(?:someone|somebody|a stranger)(?: else)?(?: has| is| keeps| has been)? (?:changed?|changing|replaced|altered|stolen|using|access(?:ed|es|ing)?|logged (?:in to|into)|(?:took|taken|taking) over)\s+" + _ACCOUNT_OBJECT,
    ...
    "churn": (
        ...
        r"\b(?:(?:i|we)'ll|(?:i|we) (?:will|intend to|plan to|am going to|are going to))\s+(?:cancel|leave|quit)\s+(?:(?:my|our|the|this|your)\s+)?(?:spotify|premium|subscription|membership|account|service|platform)\b",
        r"\b(?:i|we) (?:have |just )?cancell?ed (?:my |our )?(?:subscription|premium|account)\b",
        r"\b(?:(?:i'm|i am|we're|we are) (?:switching|moving|going)|(?:i'll|we'll|i will|we will) (?:switch|move|go)) (?:over )?to (?:apple music|tidal|deezer|youtube music|amazon music|pandora)\b",
        r"^\s*(?:switching|moving) (?:over )?to (?:apple music|tidal|deezer|youtube music|amazon music|pandora)\b",
    ...
    "support_requested": (
        ...
        r"\b(?:want|need|request|(?:i'd|i would|we'd|we would) like) (?:a |an )?(?:human|real person|support agent)\b",
...
_HARM_NEGATION = re.compile(
    r"\b(?:not|never|no|isn't|aren't|wasn't|weren't|hasn't|haven't|didn't|doesn't|don't)\s+"
    r"(?:(?:actually|really|ever|cause[ds]?|any|make|made|me|us|my|our|become|going|to|go|being|been)\s+){0,5}$",
    re.I,
)


def _preventive_only(prefix: str, suffix: str) -> bool:
    """General protection questions do not establish an incident.

    Actual-event markers prevent a protection question from hiding the incident
    it describes: 'How do I stop this after someone took over my account?'.
    """
    prevention = re.search(
        r"\b(?:how (?:can|do|should) (?:i|we|you)|ways to)\b[^.!?;]{0,45}\b"
        r"(?:prevent|avoid|protect(?: myself| my (?:account|hearing|speakers))? (?:against|from))\b[^.!?;]{0,55}$",
        prefix, re.I,
    )
    if not prevention:
        return False
    # A timing word about a later benign operation (after logout/password
    # change) is not proof that the risk event actually occurred. A causal
    # connector introducing the matched risk event, or a directly dated event,
    # is different from that general prevention context.
    prevention_tail = re.split(r"\b(?:prevent|avoid|against|from)\b", prevention.group(), flags=re.I)[-1]
    if re.search(r"\b(?:after|since|because|now that)\b[^.!?;]{0,45}$", prevention_tail, re.I):
        return False
    event_clause = re.split(r"\b(?:after|since|because|now that)\b|[.!?;]", suffix, maxsplit=1, flags=re.I)[0]
    return not bool(re.search(r"\b(?:already|yesterday|last night|just happened)\b", event_clause, re.I))


def _benign_product_harm(text: str, match: re.Match[str]) -> bool:
    harm_start = match.start("harm")
    before_harm = text[max(0, harm_start - 130):harm_start]
    before_product = text[max(0, match.start() - 50):match.start()]
    # A general present-tense possibility question does not allege an incident.
    # Specific/suspected past harm ('Could that ad have damaged my hearing?')
    # deliberately does not satisfy this narrow pattern.
    if (re.search(r"\bcan (?:loud |any |some )?$", before_product, re.I)
            and not re.search(r"\b(?:this|that|did|have|damaged|blew)\b", match.group(0), re.I)):
        return True
    source_to_harm = text[match.start():harm_start]
    if re.search(r"\b(?:when|while) (?:i|we|someone)\b", source_to_harm, re.I):
        return True
    if re.search(r"\b(?:help|cope|deal|live|relax|recover)\b[^.!?;]{0,25}\b(?:with|through|after)\s*$", before_harm, re.I):
        return True
    if re.search(r"\b(?:not|isn't|wasn't) (?:loud|strong) enough to\s*$", before_harm, re.I):
        return True
    if _HARM_NEGATION.search(before_harm):
        # A corrective complaint can express feared injury using mitigation or
        # an unwanted conditional outcome; that is not a denial of the concern.
        concern = re.search(r"\b(?:prefer|rather|don't want|do not want|before|so that)\b", before_harm, re.I)
        loudness = re.search(r"\b(?:quieter|too loud|deafening|blaring|blasting|excessive volume)\b", match.group(0), re.I)
        if not (concern and loudness):
            return True
    return False
...
def _benign_context(kind: str, text: str, match: re.Match[str]) -> bool:
    ...
    if "harm" in match.re.groupindex and _benign_product_harm(text, match):
        return True
    ...
    if kind in {"security", "legal_safety"}:
        if _preventive_only(prefix, text[match.start():match.end() + 60]):
            return True
    if kind == "repeated_contact":
        ...
        if re.search(r"\b(?:fixed|resolved|sorted) (?:it|the (?:problem|issue))\b", suffix, re.I) and not re.search(r"\b(?:not|never|didn't)\b", suffix, re.I):
            return True
    if kind == "churn":
        if re.search(r"\bleav(?:e|ing) (?:spotify|the (?:service|platform))$", phrase, re.I) and re.match(
            r"\s+(?:open|running|playing|in the background)\b|\s+on(?=\s*$|\s*[.!?])", text[match.end():], re.I
        ):
            return True
...
def deterministic_risks(customer_text: str) -> tuple[RiskFinding, ...]:
    ...
    for kind in POLICY["risk_priority"]:
        for pattern in _COMPILED[kind]:
            match = _first_asserted_match(kind, pattern, text)
...
def _first_asserted_match(kind: str, pattern: re.Pattern[str], text: str) -> re.Match[str] | None:
    """Inspect every alleged harm even when the first one is explicitly denied.

    A product can be the subject of two claims: 'the ad did not hurt my ears but
    damaged my speakers'. Mask only the dismissed harm span and search again
    from its original subject. Offsets still refer to the unchanged customer
    text; no previously established hard risk is removed.
    """
    working = text
    position = 0
    while match := pattern.search(working, position):
        if not _benign_context(kind, text, match):
            return match
        if "harm" in match.re.groupindex:
            start, end = match.span("harm")
