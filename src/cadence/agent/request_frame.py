...
ISSUES: tuple[str, ...] = (
    ...
    "connect_unwanted_takeover", "playlist_selection", "resume_position", "feedback_submission",
)
SLOTS: tuple[str, ...] = (
    ...
    "workflow_request", "catalog_absence", "accounts_used",
)
...
Unwanted automatic switching/takeover of playback is connect_unwanted_takeover,
not permission to explain intentional device selection. Distinguish a known
household device behaving unexpectedly from an unknown actor/device; preserve
any security ambiguity. A missing track within a named/editorial playlist is
playlist_selection, not evidence that the recording is absent from the catalog.
resume_position describes failure/questions about resuming the current playback
position (including an audiobook); do not assume it requests a future feature.
Whether a capability currently exists must be established by current knowledge,
not by turning the customer's belief or uncertainty into a known availability fact.
feedback_submission means an explicit question about WHERE/HOW to submit feedback
or an idea; it is a public_howto process request. Merely expressing a feature wish
does not establish that the customer requested a submission-workflow explanation.
...
workflow_request quotes an explicitly requested process (e.g. where to submit an
idea); never infer it merely because the customer expressed an idea or complaint.
catalog_absence quotes the CUSTOMER'S claim or question that a recording is absent
from Spotify's service-wide catalog, not independently verified availability.
Omission from one named/editorial playlist does not support this slot. accounts_used
records an explicitly described shared account or separate accounts, with its quote;
mentioning a family or multiple people alone does not establish account setup.
...
- legal_safety: actual allegation/threat of discrimination, harassment, legal
  action, self-harm or harm to others. Include alleged product-caused physical,
  hearing or property damage and a complaint that the current product behavior
  threatens such harm. A request to reduce harmful behavior may express the
  concern politely, indirectly or conditionally; it need not mention a lawyer
  or prove injury. Do not dismiss that concern as merely an app fault or feature
  suggestion. Distinguish figurative praise, quoted titles, explicit denial of
  harm and general hypothetical prevention questions from actual allegations.
  A question or an 'if' clause may still report real harm or a current threat;
  do not suppress risk just because it is phrased that way.
- security: actual suspected compromise, unauthorized changes/access, disclosed
  secrets or someone taking over an account. Identify the object being changed:
  a UI layout or playlist changing is not account takeover. General prevention
  or a public profile how-to is not compromise; a prevention question that ALSO
  reports an actual compromise still contains a security incident.
...
- churn: explicit intent/threat to leave/cancel or switch providers; a neutral
  question about public cancellation terms alone is not churn. Resolve what is
  being cancelled or left: cancelling a download or leaving a playlist/app open
  is not leaving the service. Contracted intent ('I'll', 'we'll') still counts.
