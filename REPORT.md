...
The new confirmation recovers raw automatic coverage to 30/80 (37.5%), versus 28/80 for reference and 7/80 for quality. Balanced and reference each miss four of 38 required escalations; quality misses none. Thus the selected candidate **does not achieve the fixed coverage-and-safety objective** and remains experimental. Its 386,582 input/output tokens are 1.88 times the reference's 205,542. The [four routing misses](docs/BALANCED_FAILURES.md) expose frustration, discrimination, ambiguity and account-identity policy boundaries. No code or label was retuned after those outcomes.
