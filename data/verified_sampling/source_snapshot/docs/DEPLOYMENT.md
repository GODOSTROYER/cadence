...
**Experimental implementation update:** `VerifiedAgent` is available for controlled evaluation and is not selected by the public API. Its prospective policy, current-source registry and new evaluation packets are documented in [Verified design](VERIFIED_DESIGN.md). Deployment of this code does not promote the candidate or replace historical dashboard metrics. Runtime health identifies the served revision.
