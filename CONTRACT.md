...
## Verified experimental architecture addendum — 18 September 2026

The separate `VerifiedAgent` implements the accepted plan in `output/analysis/Cadence-Implementation-Plan.md`. It preserves the response API and adds `VerifiedTrace` fields for request facts, policy/knowledge hashes, exact alternatives, veto stages, source dates and shared attempt/usage budgets. Historical comparator behavior and artifacts are unchanged. See `docs/VERIFIED_DESIGN.md` and `docs/VERIFIED_EVALUATION.md` for prospective policy, metrics, review and promotion requirements.

Official Spotify article retrieval is additionally authorized for offline knowledge verification through `scripts/verify_knowledge.py --online`; it is never part of customer request handling. Stored source snapshots and explicit reviews bind present-day guidance. Experimental sampling uses manifest-recorded seeds and records known historical exposure limitations. Final confirmation and challenge inference require their own actual human review records. AI-authored scores are never converted into human scores by the tools.
