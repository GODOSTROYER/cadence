...
import { BalancedStudy } from "@/components/BalancedStudy";
...
interface Benchmark {
  ...
  routing_experiment: { n: number; current: { escalation_recall: number; auto_handle_rate: number }; selective: { escalation_recall: number; auto_handle_rate: number }; useful_coverage: {agent: {useful_automatic_coverage: number}; selective: {useful_automatic_coverage: number}}; source: string };
  quality_confirmation: {n: number; metrics: {left: {escalation_recall: number; auto_handle_rate: number}; right: {escalation_recall: number; auto_handle_rate: number}}; useful_coverage: {agent: {useful_automatic_coverage: number}; quality: {useful_automatic_coverage: number}}; acceptance: string; source: string};
}
...
export function BenchmarkOverview() {
  ...
  return <PageTransition className="flex flex-col gap-12 md:gap-16">
    ...
    <section className="grid gap-8 md:grid-cols-2">
      ...
      <div><p className="eyebrow mb-3">05 · routing development reviewed</p><h2 className="t-display text-3xl">Automatic is not always useful.</h2><p className="mt-4 leading-relaxed text-muted">On {data.routing_experiment.n} development messages, selective routing achieved {percent(data.routing_experiment.selective.escalation_recall)} escalation recall at {percent(data.routing_experiment.selective.auto_handle_rate)} automatic coverage. Astra’s exact-reply review found useful automatic coverage of only {percent(data.routing_experiment.useful_coverage.selective.useful_automatic_coverage)}, versus {percent(data.routing_experiment.useful_coverage.agent.useful_automatic_coverage)} for the reference. New reviews retain AI provenance.</p><a className="link mt-3 inline-block text-sm" href={`${REPO}${data.routing_experiment.source}`}>Development comparison</a></div>
    </section>
    <section className="rounded-lg border border-border p-6 md:p-8"><p className="eyebrow mb-3">06 · fresh confirmation completed</p><h2 className="t-display text-3xl">Approved actions, tested on unseen cases.</h2><p className="mt-4 max-w-4xl leading-relaxed text-muted">The quality candidate chooses from verified response actions and checks whether the exact reply fits the issue. On {data.quality_confirmation.n} unseen messages, its escalation recall was {percent(data.quality_confirmation.metrics.left.escalation_recall)}, with {percent(data.quality_confirmation.useful_coverage.quality.useful_automatic_coverage)} useful automatic coverage; the reference achieved {percent(data.quality_confirmation.metrics.right.escalation_recall)} recall and {percent(data.quality_confirmation.useful_coverage.agent.useful_automatic_coverage)} useful coverage. Automatic coverage fell from {percent(data.quality_confirmation.metrics.right.auto_handle_rate)} to {percent(data.quality_confirmation.metrics.left.auto_handle_rate)}. These new labels and reply reviews are AI-authored; the new reviews await human verification. {data.quality_confirmation.acceptance}</p><a className="link mt-4 inline-block text-sm" href={`${REPO}${data.quality_confirmation.source}`}>Acceptance results</a><a className="link ml-6 mt-4 inline-block text-sm" href="https://github.com/GODOSTROYER/cadence/raw/refs/heads/main/output/pdf/Cadence-Arnav-Bule-Report.pdf">Six-page PDF report</a></section>
    <BalancedStudy />
