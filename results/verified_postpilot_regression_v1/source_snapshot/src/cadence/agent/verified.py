...
from cadence.agent.policy_v2 import POLICY, POLICY_VERSION, evaluate_policy
...
from cadence.config import SPOTIFY_HANDLES, Paths
...
Apply the supplied frozen policy boundaries. Ordinary frustration, criticism,
a general request for help, or a public offer question alone is not a human
takeover request or a money incident. Do not invent additional mandatory routes.
...
Distinguish a named function or observable symptom from an unspecified complaint.
An ambiguous request cannot be assigned a playback fault or feature proposal just
because one of the available replies would fit that interpretation. Feature
proposals are not evidence that an existing feature is malfunctioning.
...
class VerifiedTrace(Trace):
    ...
    audit_risk_reason: str | None = None
...
class VerifiedAgent(SupportAgent):
    ...
    def __init__(
        ...
        self.knowledge = knowledge if knowledge is not None else KnowledgeStore.load(
            Paths.CONFIG / "knowledge" / "v3.json"
        )
                    ...
                    if policy.required:
                        ...
                        if not candidates:
                            ...
                            payload = {
                                ...
                                "locked_policy": {
                                    "required": False, "version": trace.policy_version,
                                    "sha256": trace.policy_sha256,
                                    "boundaries": POLICY["boundaries"],
                                    "mandatory_risks": POLICY["risks"],
                                },
                                "alternatives": [
                            ...
                            trace.audit_risk_reason = audit.risk_reason
