...
FACTS = {
    "family": ("family-plan/", "Premium Family is for family members living together. Members must live with the plan manager and use the same home address. This general rule does not establish a temporary-absence exception or eligibility of an individual account."),
    "wrapped": ("spotify-wrapped/", "Wrapped is Spotify's annual listening summary and includes personalized playlists. Some features have eligibility requirements. The support page links official updates; it does not establish a release date for an upcoming edition."),
...
action('profile_name', "Your display name is what appears on your profile and playlists. You can edit it without changing your login details: " + BASE + 'username-and-display-name/', 'current:profile', 'General question about changing the visible display/profile name. Never use for an assigned/generated username or underlying account-identifier change, recovery or a hacked account.')
action('family_household', "Premium Family members must live with the plan manager at the same address. I can't confirm an exception for time abroad; the household rule is here: " + BASE + 'family-plan/', 'current:family', 'General Family household eligibility question; no rejected invitation, verification failure, billing or request to approve an account.')
action('wrapped_updates', "Wrapped is Spotify's annual listening summary, including personalized playlists. I can't confirm a release date; check the official updates linked here: " + BASE + 'spotify-wrapped/', 'current:wrapped', 'General question about annual listening summary or top-songs playlist timing; not missing account-specific results or a feature proposal.')
...
action('ask_playback_mode', "Do the extra songs play after your playlist ends, or between its tracks? Are you using Free or Premium?", 'policy:clarification', 'Unexpected recommended songs with both playback timing and plan missing. Ask only if neither requested detail was already supplied.')
...
RISK_CODES = {'account_identity_change': 'needs_account_lookup',
              'failed_student_verification': 'needs_account_lookup', 'out_of_scope': 'out_of_scope'}
PAST_CHURN = re.compile(r'\b(?:cancel(?:led|ed)|quit|left|stopped using)\s+(?:(?:my|the|your)\s+)?(?:premium|subscription|spotify|service|account)\b', re.I)
NEGATED_ACTION = re.compile(r"\b(?:not|never|haven['’]t|hasn['’]t|hadn['’]t|didn['’]t|wouldn['’]t)(?:\s+\w+){0,2}\s*$", re.I)

Plan = create_model('BalancedPlan', __base__=RoutingDecision,
    request_scope=(Literal['public_help', 'account_identity_change', 'failed_student_verification', 'out_of_scope', 'other_human_required'], Field(description='Identify the customer request before selecting an answer. Changing an assigned account identifier is account_identity_change; changing a visible display name or proposing a feature is public_help. A system unable to verify student status is failed_student_verification, not browser trouble. Pure social announcements without support requests are out_of_scope. Other policy-required intervention is other_human_required.')),
...
class Audit(BaseModel):
    reason: str = Field(description='First identify the actual customer request, known/missing details, and the exact proposed wording that supports or contradicts the selected action. Do not attribute claims to a reply that it explicitly declines to make.')
...
def identifier_request(text):
    # This display-name-only card cannot answer an underlying username request or contrast.
    return bool(re.search(r'\buser\s?name\b', text, re.I))


def completed_churn(text):
    return any(not NEGATED_ACTION.search(text[max(0, match.start() - 50):match.start()])
               for match in PAST_CHURN.finditer(text))
...
class BalancedAgent(SupportAgent):
    ...
    def _prepare(self, text, rules, exclude_thread_ids):
        # The archived shared rule misses past-tense churn; add a veto for this candidate only.
        if completed_churn(text):
            rules.flags.append('completed_churn')
            if not rules.force_escalate:
                rules.force_escalate, rules.reason_code = True, 'high_frustration_or_churn'
                rules.reason = 'Customer reports leaving or cancelling the service; a human should handle the complaint.'
        ...
        plan, first = self.client.generate_json(json.dumps(payload), Plan,
            ...
            'When an answer needs unknown plan/device/timing, select a necessary clarification instead, and make the fallback '
            'a different valid question rather than an inapplicable procedure. Playlist-state bugs can need device/app details. '
            'Missing diagnostic details about an identified feature are NOT an unintelligible issue. A screenshot link '
            'alongside a described app fault is NOT a media-only message: ask for its exact error text. '
            'A request to improve a feature can use the Ideas route without a roadmap promise; ordinary disappointment is '
            'not itself severe frustration. Yearly listening summaries are Wrapped, not requests for a new feature. '
        ...
        if plan.action == 'profile_name' and identifier_request(text):
            plan = plan.model_copy(update={'request_scope': 'account_identity_change'})
        # A convenient public answer cannot erase the nature of the original request.
        if plan.request_scope != 'public_help':
            fixed.update(decision='escalate', escalation_reason_code=RISK_CODES.get(plan.request_scope, plan.escalation_reason_code or 'needs_account_lookup'),
                         escalation_reason='Request requires escalation: ' + plan.request_scope)
            plan = plan.model_copy(update={k: fixed[k] for k in ('decision', 'escalation_reason_code', 'escalation_reason')})
        handoff = 'student_handoff' if (
            (plan.request_scope == 'failed_student_verification' or
             (plan.request_scope == 'public_help' and plan.action in {'student_browser', 'student_handoff'})) and not rules.force_escalate
        ...
        audit, second = self.client.generate_json(json.dumps({'customer': text, 'proposed_replies': alternatives,
            ...
            'satisfies every condition. An Ideas-board link with an explicit no-date/no-forwarding limitation makes '
            'no operational promise and can help feature feedback; do not demand that it implement the feature. '
            'Missing plan/device information can justify a targeted question, not account access by itself. '
            'General household rules can answer a general eligibility question without approving a specific account. '
            'A sentence saying I cannot confirm a date does not assert a date. Check actual wording, not a hypothetical '
            'unsafe answer. Missing context for a known technical issue can be requested; a screenshot with described '
            'symptoms is not media-only. Requesting device/app details for a playlist-state fault can be useful. '
            'Otherwise select handoff.\n' + policy_block())
        ...
        if selected == 'profile_name' and identifier_request(text):
            safe = False
            audit.reason = 'Account-identifier request cannot be resolved by a display-name alternative.'
