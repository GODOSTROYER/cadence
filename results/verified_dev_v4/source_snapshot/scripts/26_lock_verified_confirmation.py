"""Lock cases disjoint from recorded exposures and import human labels before inference.

An explicit exposure registry is mandatory. Paths resolve relative to the registry:
{"version": 1, "known_exposure_inventory_complete": true, "exposure_limitations": [],
...
not something software can independently establish. Disjointness applies to recorded
and reconstructed exposures; missing historical inspection records remain a limitation.
"""
...
def load_registry(path: Path, required_exposures: set[str] | None = None) -> tuple[dict, list[dict], list[dict]]:
    ...
    if registry.get("version") != 1 or registry.get("known_exposure_inventory_complete") is not True:
        raise ValueError("Registry v1 must attest its known exposure inventory is complete")
    if "all_inspected_sets_included" in registry:
        raise ValueError("Replace the absolute exposure attestation with known_exposure_inventory_complete")
    limitations = registry.get("exposure_limitations")
    if not isinstance(limitations, list) or any(not isinstance(item, str) or not item.strip() for item in limitations):
        raise ValueError("exposure_limitations must be a list of nonempty limitation strings")
    scope = registry.get("inventory_scope", "Recorded and reconstructed exposures declared in this registry")
    if not isinstance(scope, str) or not scope.strip():
        raise ValueError("inventory_scope must be a nonempty description when supplied")
...
def _write_partition(exposure_registry: Path, registry: dict, out: Path, policy: Path,
    ...
    lock = {
        ...
        "population": "English brand-replied corpus threads disjoint from recorded/reconstructed exposures",
        "sampling": "seeded order; transitive tweet components and casefold fuzzy ratio >=85 excluded",
        "limitations": "Distinct complaint sample, not traffic-weighted; unrecorded historical exposure remains possible",
        "known_exposure_inventory_complete": True,
        "inventory_scope": registry.get("inventory_scope", "Recorded and reconstructed exposures declared in this registry"),
        "exposure_limitations": list(registry["exposure_limitations"]),
...
def prepare_challenge(exposure_registry: Path, out: Path, policy: Path, cases: Path,
    ...
    return _write_partition(exposure_registry, registry, out, policy, examples, "challenge", None,
        ...
        "sampling": "Authored cases checked against recorded/reconstructed prior and calibration exposures; no random boundary claim",
        "limitations": "Oversampled boundary suite; not a representative coverage estimate; paired cases dependent; unrecorded historical exposure remains possible",
