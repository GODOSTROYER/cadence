...
action('explicit_version', "Search for another version of the track and check its E or EXPLICIT tag. Versions depend on what rights holders provide; a different version may not be available: " + BASE + 'explicit-content/', 'current:explicit', 'Customer wants an unedited/clean version, not an incorrectly tagged release or an account restriction.')
...
class BalancedAgent(SupportAgent):
    ...
    def _prepare(self, text, rules, exclude_thread_ids):
        ...
        if policy_decision == 'escalate' or plan.action == 'handoff':
            ...
            draft = LLMDecision.model_validate({**fixed, 'reply_draft': render('handoff'),
                'citations': ['current:contact'], 'grounding_notes': plan.rationale})
            return [SOURCE['current:contact'].model_copy(deep=True)], draft, first, 0, 1
        ...
        if audit.human_required or not safe or selected == 'handoff':
            selected = 'handoff'
