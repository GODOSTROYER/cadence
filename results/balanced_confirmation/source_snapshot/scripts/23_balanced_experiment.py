...
class InvocationClient:
    """Record every call's cache status so resumed partial invocations cannot look fast."""
    ...
    def generate_json(self, *args, **kwargs):
        result, meta = self.client.generate_json(*args, **kwargs)
        self.cached.append(bool(meta.cached))
        return result, meta
...
def main():
    ...
    if args.phase == "confirmation":
        ...
        review = read_json(args.labels.parent / 'LABEL_REVIEW.json')
        if (review.get('status') != 'accepted_for_pre_inference_evaluation'
                or review.get('labels_sha256') != sha256(args.labels)
                or review.get('sample_sha256') != lock['examples_sha256']):
            raise ValueError('Confirmation annotation review does not bind these labels')
        for name, digest in {**lock['sources'], **review['reference_policy_sha256']}.items():
            if sha256(Paths.ROOT / name.replace('\\', '/')) != digest:
                raise ValueError('Confirmation sampling or annotation dependency changed: ' + name)
    ...
    frozen = {"inputs": {str(f.resolve().relative_to(Paths.ROOT)).replace('\\', '/'): sha256(f) for f in watched},
              ...
              "systems": ["agent", "quality", "balanced"], "label_sources": sorted({r['label_source'] for r in labels}),
              "acceptance": {"no_more_missed_escalations": True, "strictly_better_useful_coverage_than_both": True, "no_more_misses_than_either": True,
                             "min_auto_coverage": .20, "max_auto_coverage_loss_vs_reference": .10,
                             "no_fewer_useful_resolutions_than_reference": True,
                             "no_reviewer_flagged_unsafe_automatic": True, "max_candidate_p95_ms": 7500,
                             "latency_observations_fully_fresh": True,
    ...
    if not manifest.exists():
        ...
        for file in watched:
            if file.suffix == '.py':
                target = args.out / 'source_snapshot' / file.resolve().relative_to(Paths.ROOT)
    ...
    client = GeminiClient(model_name('agent'), cache_path=args.out / 'calls.sqlite', deadline_s=120)
    measured = InvocationClient(client)
    agents = {'agent': SupportAgent(measured, retriever, threshold=.9),
              'quality': QualityAgent(measured, retriever, threshold=.9, system_name='quality'),
              'balanced': BalancedAgent(measured, retriever, threshold=.9, system_name='balanced')}
    ...
    try:
        for i, label in enumerate(labels):
            ...
            for system in order:
                ...
                row['measurement'] = {'per_call_cached': list(measured.cached),
                                      'fully_fresh_invocation': not any(measured.cached)}
        ...
        write_json(args.out / 'INTERRUPTION.json', {'status': 'interrupted', 'completed': len(rows),
            'exception_type': type(error).__name__, 'partial_invocation_cache_flags': measured.cached,
            'latency_note': 'Aborted invocation time is excluded. A resumed mixed-cache invocation is ineligible for latency acceptance.'})
        raise
    finally:
        client.close()
    ...
    summary = {'status': 'complete', 'phase': args.phase, 'n': len(labels), 'label_sources': frozen['label_sources'],
               'comparisons': {s: compare(labels, grouped[s], grouped['agent']) for s in ('quality', 'balanced')},
               'balanced_vs_quality': compare(labels, grouped['balanced'], grouped['quality']),
               'runtime': {s: {'n': len(rs), 'p50_ms': float(np.median([r['latency_ms'] for r in rs])),
                               ...
                               **{key: sum(r['trace'][key] for r in rs) for key in ('model_calls', 'prompt_tokens', 'output_tokens')},
                               ...
                               'fresh_invocations': sum(r['measurement']['fully_fresh_invocation'] for r in rs)} for s, rs in grouped.items()},
