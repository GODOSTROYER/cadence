...
def scenario_for(label):
    """The human packet preserves setup; allow its explicit CSV alias only consistently."""
    if "setup" in label and "scenario_setup" in label and label["setup"] != label["scenario_setup"]:
        raise ValueError("Conflicting operational setup fields")
    return label.get("scenario_setup", label.get("setup", {}))


def persist_outcome(directory, rows, row, expected, failure_limit, *, expected_fault=False):
    """Save outcomes before stopping repeated failures; never seal a partial cohort."""
    write_jsonl(directory / "predictions.jsonl", [row], append=True)
    rows.append(row)
    status = {"status": "partial", "completed": len(rows), "expected": expected}
    write_json(directory / "status.json", status)
    failures = 0
    for old in reversed(rows):
        if old.get("outcome") == "success" or old.get("challenge", {}).get("triggered"):
            break
        failures += 1
    if not expected_fault and failures >= failure_limit:
        write_json(directory / "INTERRUPTION.json", {**status, "reason": "consecutive_failed_invocations",
            "consecutive_failures": failures, "stopped_at": utcnow(),
            "note": "Every attempted outcome is preserved. Remaining locked messages were not attempted; no complete-result claim."})
        raise RuntimeError("Circuit breaker stopped repeated failed invocations; inspect preserved error and usage records before resuming")
...
def invocation(agent, client, label, excluded, *, seconds, max_attempts):
    ...
    try:
        with challenge_setup(agent, client, scenario_for(label)) as challenge:
    ...
    row["scenario_setup"] = scenario_for(label)
    return row
...
def freeze(args, labels):
    ...
    frozen = {"inputs": inputs, "labels": relative(args.labels), "policy": relative(args.policy),
              ...
              "reserved_samples": sorted(relative(p) for p in args.reserved_sample),
...
def main():
    ...
    parser.add_argument("--reserved-sample", type=Path, action="append", default=[],
                        help="Repeat for locked future samples excluded from retrieval; exact bytes are frozen")
    parser.add_argument("--live-free-tier", action="store_true")
    parser.add_argument("--freeze-only", action="store_true")
    args = parser.parse_args()
    if args.deadline <= 0 or args.max_attempts <= 0 or args.max_consecutive_failures <= 0:
    ...
    reserved_paths = {p.resolve() for p in args.reserved_sample}
    if len(reserved_paths) != len(args.reserved_sample) or any(not p.is_relative_to(Paths.ROOT.resolve()) or not p.is_file() for p in reserved_paths):
        raise ValueError("Reserved samples must be unique existing files inside the repository")
    final_sample = Paths.DATA / "verified_confirmation/examples.jsonl"
    if args.phase in ("development", "calibration") and final_sample.exists() and final_sample.resolve() not in reserved_paths:
        raise ValueError("The locked confirmation sample must be supplied as --reserved-sample before development/calibration retrieval")
    ...
    for row in labels:
        setup = validate_setup(scenario_for(row), args.systems)
    ...
    reserved = [r for path in args.reserved_sample for r in read_jsonl(path)]
    if any(not isinstance(r.get("text"), str) for r in reserved):
        raise ValueError("Reserved samples require customer text for local duplicate exclusion")
    excluded = excluded_retrieval(threads, [*labels, *reserved])
    ...
    try:
        for i, label in enumerate(labels):
            ...
            for system in order:
                ...
                persist_outcome(args.out, rows, row, len(expected), args.max_consecutive_failures,
                                expected_fault=args.phase == "challenge" and row.get("challenge", {}).get("triggered", False))
