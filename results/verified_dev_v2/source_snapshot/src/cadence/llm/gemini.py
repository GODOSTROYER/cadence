...
@dataclass
class RequestBudget:
    """Safe aggregate accounting for one optional multi-call request scope.

    Token totals include every provider response with available usage, including
    responses that failed parsing. Unknown usage must not be treated as zero cost,
    even when the response was usable. Cached calls do not contribute to any of
    these counters.
    """
    ...
    unknown_failed_usage: bool = False
    deadline_exceeded: bool = False

    def as_dict(self) -> dict[str, Any]:
        """Return counters only; no prompts, credentials or provider errors."""
        return asdict(self)
...
def _has_additional_properties(schema: Any) -> bool:
    """Whether any schema node needs JSON Schema's additionalProperties keyword."""
    if isinstance(schema, dict):
        return "additionalProperties" in schema or any(_has_additional_properties(v) for v in schema.values())
...
class GeminiClient:
    ...
    # --------------------------------------------------------- request budget
    @contextmanager
    def request_budget(self, seconds: float, max_attempts: int = 4) -> Iterator[RequestBudget]:
        """Share a deadline and provider-attempt limit across structured calls.

        State is local to the calling thread. A nested scope can tighten the
        enclosing limits, but cannot reset them: provider attempts and usage are
        charged to every active scope. The yielded record remains readable after
        the scope exits, including when a call raised an exception.
        """
        if not math.isfinite(seconds) or seconds <= 0:
            raise ValueError("request budget seconds must be finite and positive")
        if isinstance(max_attempts, bool) or not isinstance(max_attempts, int) or max_attempts < 1:
            raise ValueError("request budget max_attempts must be a positive integer")
        parents = getattr(self._request, "budgets", ())
        deadline = min([self._clock() + seconds, *(b.deadline for b in parents)])
        budget = RequestBudget(deadline=deadline, max_attempts=max_attempts)
        self._request.budgets = (*parents, budget)
        try:
            yield budget
        finally:
            self._request.budgets = parents

    def budget_status(self) -> dict[str, Any] | None:
        """Current thread's innermost budget, or None outside a request scope."""
        budgets = getattr(self._request, "budgets", ())
        return budgets[-1].as_dict() if budgets else None

    def _check_request_budget(self) -> None:
        budgets = getattr(self._request, "budgets", ())
        now = self._clock()
        expired = [budget for budget in budgets if now >= budget.deadline]
        for budget in expired:
            budget.deadline_exceeded = True
        if expired:
            raise QuotaExhausted("Gemini shared request deadline exceeded; retry later.")
        if any(budget.network_attempts >= budget.max_attempts for budget in budgets):
            raise LLMError("Gemini shared request attempt budget exhausted.")

    def _consume_request_attempt(self) -> None:
        self._check_request_budget()
        for budget in getattr(self._request, "budgets", ()):
            budget.network_attempts += 1

    def _record_request_usage(self, response: Any, *, failed: bool) -> None:
        budgets = getattr(self._request, "budgets", ())
        if not budgets:
            return
        ...
        usage_complete = (
            getattr(usage, "prompt_token_count", None) is not None
            and getattr(usage, "candidates_token_count", None) is not None
        )
        for budget in budgets:
            ...
            if failed:
                budget.failed_attempts += 1
                if not usage_complete:
                    budget.unknown_failed_usage = True
    ...
    def _config(self, schema: type[BaseModel], system: str | None, temperature: float) -> Any:
        ...
        # The SDK's response_schema converter can serialize additionalProperties
        # as additional_properties, which the API rejects. Pass these schemas
        # through the supported JSON Schema field; retain the historical schema
        # path for existing callers. _parse still validates the original model.
        schema_config = (
            {"response_json_schema": json_schema}
            if _has_additional_properties(json_schema)
    ...
    def _attempt(
        ...
        try:
            ...
            config = self._config(schema, system, temperature)
            self._consume_request_attempt()
            attempted = True
            response = provider.models.generate_content(model=self.model, contents=prompt, config=config)
            obj = self._parse(response, schema)
            # A transport may return after its timeout. Do not report such a
            # response as a successful call within an explicitly shared budget.
            budgets = getattr(self._request, "budgets", ())
            expired = bool(budgets) and self._clock() >= self._request.deadline
            for budget in budgets if expired else ():
                budget.deadline_exceeded = True
            if expired:
                raise QuotaExhausted("Gemini shared request deadline exceeded; retry later.")
            succeeded = True
            ...
            if attempted:
                self._record_request_usage(response, failed=not succeeded)
    ...
    def _call_with_retry(
        ...
        previous_deadline = getattr(self._request, "deadline", None)
        deadlines = [self._clock() + self.deadline_s]
        deadlines.extend(b.deadline for b in getattr(self._request, "budgets", ()))
        self._request.deadline = min(deadlines)
        try:
            return self._retry_with_deadline(prompt, schema, system, temperature)
        finally:
            if previous_deadline is None:
                del self._request.deadline
            else:
                self._request.deadline = previous_deadline
