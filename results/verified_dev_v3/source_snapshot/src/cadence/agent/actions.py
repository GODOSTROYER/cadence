...
TECHNICAL = PLAYBACK_FAILURES | {"app_problem"}
...
ACTION_SPECS = (
    ...
    _spec("ask_playback_scope", PLAYBACK_FAILURES, "clarification"),
    ...
    _spec("ask_catalog_context", {"catalog_missing"}, "clarification", forbidden={"track_unplayable"}),
...
def _technical_context(frame) -> bool:
    return bool(_issues(frame) & PLAYBACK_FAILURES or
                "app_problem" in _issues(frame) and _concrete_app_failure(frame))
...
def _clarification_dependency(spec, frame, text, knowledge, as_of):
    ...
    if target is None or spec.id == "ask_device" and _technical_context(frame):
        return True, [], [], ()
...
def _contract(spec: ActionSpec, frame, text: str, *, allow_missing: frozenset[str] = frozenset()) -> tuple[bool, list[str]]:
    ...
    social_eligible = spec.id == "social_thanks" and frame.social_closure and not issues - {"social_closure"}
    matched = issues & spec.issues
    if (not matched and not social_eligible) or issues & spec.forbidden_issues:
        return False, []
    if matched == {"app_problem"} and not _concrete_app_failure(frame):
        return False, []
    ...
    } and not (spec.id == "spotify_connect" and not issues & PLAYBACK_FAILURES):
        return False, []
    name = spec.id
    ...
    if name == "ask_device" and _known(frame, "device") and (
        ...
        and not _technical_context(frame)
    ):
        return False, []
    ...
    if name == "autoplay_off" and not re.search(
        ...
        return False, []
    if name == "feature_idea" and (frame.request_kind != "feature_change" or not _known(frame, "requested_change")
                                   or _public_howto(frame, text) or _known(frame, "error")):
        return False, []
...
def _render(spec: ActionSpec, frame, url: str, text: str = "") -> str:
    ...
    if name == "ask_device":
        ...
        if _issues(frame) & CONTROLS or not _technical_context(frame) and (
...
def shortlist(frame: RequestFrame, text: str, knowledge: KnowledgeStore, max_choices: int = 4,
    ...
    if _public_howto(frame, text) and not _technical_context(frame):
