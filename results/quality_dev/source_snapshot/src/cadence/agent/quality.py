...
class QualityAgent(SupportAgent):
    ...
    def _prepare(self, text, rules, exclude_thread_ids):
        ...
        draft, meta = self.client.generate_json(json.dumps(payload), LLMDecision,
            system="Draft a brief public Spotify support reply ending ' /AI', at most 280 characters. "
                   ...
                   "A safe handoff is not an automatic resolution. Use supplied route fields unchanged unless escalating.")
        metas.append(meta)
        ...
        draft = draft.model_copy(update=fixed)
