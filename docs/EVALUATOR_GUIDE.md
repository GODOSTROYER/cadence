...
Use the [evaluation workflow](VERIFIED_EVALUATION.md) for fresh run names and required reserved-sample flags, and [ablation guide](VERIFIED_ABLATIONS.md) for offline diagnostic controls. Read-only verification of an existing saved candidate summary uses `python analysis_tools/verify_portable_summaries.py verified --experiment PATH_TO_RUN`; [Verified status](VERIFIED_STATUS.md#verify-the-saved-evidence) lists exact paths and the `development`, `variants` and `controls` commands. Only finite float-to-float summary differences up to `1e-12` absolute are allowed, with no relative tolerance; counts, types, keys, provenance and artifact hashes remain exact. Original saved evidence is unchanged. Preparing a new run or review packet is a separate operation.
