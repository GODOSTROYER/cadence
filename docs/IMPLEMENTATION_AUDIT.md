...
- The completed benchmark review and approval is by Arnav Bule, covering all 200 revised examples and their existing scores.
...
| Evaluation | `eval/paired.py: 12`, `eval/blind_judge.py: 17`, `scripts/12_run_holdout.py: 28` | Paired samples and two presentation orders; same-family judge; completed benchmark review by Arnav Bule |
...
| UI | `ui/src/` types and decision display; historical dashboard artifacts | At audit time the public site predated these changes. See [deployment verification](DEPLOYMENT.md) for the current release; charts remain historical |
...
|---|---|---|---|---|---|---|---|
| Historical AI test reused across three runs; `REPORT_historical.md` | Tune more rules on known failures | **Done:** separate frozen 200-example benchmark, rationales, hashes; no relabeling after prediction | Stops conflating debugging and confirmation | More credible generalization estimate within the sampled policy | 1–2 days; annotation quality remains uncertain | Exact ID/hash checks; publish every system and failed run; completed review by Arnav Bule | P0 |
...
| Same-family judge; resource omissions often shipped | More judge calls / new rubric | **Done:** candidate-own evidence, missing-resource penalties, both orders | Order and presentation can dominate scores | More diagnostic judge evidence | Two calls per pair; shared biases remain | Publish order disagreement and all ratings; no order cherry-picking | P0 |
...
| Fresh reply comparison |Same 200 inputs and released drafts |agent vs nearest baseline, both orders | Complete: +1.4175/5 CI [1.145,1.6675], order consistency .735; known unsafe replies still score 5/5 |
...
1. **P0 completed:** `agent/integrity.py`, `pipeline.py`, `prompts.py`, `links.py`, `config/canonical_links.json`; safety checks and scoped canonical links. `eval/paired.py`, `blind_judge.py`, `run_eval.py`; honest paired evidence. `scripts/08_*`–`13_*`, `reproduce_recorded.py`; lock, provenance, review and replay. API input/privacy/retry changes and test-output isolation. Documentation records completed human review and corrects threshold claims.
