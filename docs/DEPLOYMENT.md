...
**Experimental implementation update:** `VerifiedAgent` is available for controlled evaluation and is not selected by the public API; `SupportAgent` remains the default. [Verified status](VERIFIED_STATUS.md) reports the completed AI-reviewed development/calibration pilots and their failed safety criteria. A subsequent policy/action-scope patch and explicit knowledge v3 completed an 18-case targeted regression with zero required-route misses and zero flagged automatic replies, but only one useful clarification and seven unnecessary escalations. This is no generalization or restored-coverage claim. The new 200-case confirmation and 80-case challenge human-label packets remain untouched and blank. No candidate promotion, production routing change or new deployment verification is implied by these experiments. Runtime health identifies the served revision.

For read-only checks of the current experimental summaries, use the [portable verification commands](VERIFIED_STATUS.md#verify-the-saved-evidence), starting with `python analysis_tools/verify_portable_summaries.py development`. The wrapper retains exact counts, types, keys, provenance and artifact-hash checks; only finite float-to-float summary differences of at most `1e-12` absolute are allowed, with no relative tolerance. This changes neither saved study evidence nor deployment status.
