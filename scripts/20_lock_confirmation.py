...
def main():
    out = Paths.DATA / 'quality_confirmation'
    if out.exists():
    ...
    sources = [Paths.CANDIDATES, Paths.GOLDEN, Paths.ROOT / 'docs/taxonomy_calibration.jsonl',
               Paths.DATA / 'holdout/examples.jsonl', Paths.DATA / 'routing_dev/examples.jsonl']
    ...
    random.Random(2026091703).shuffle(candidates)
    chosen = []
    for t in candidates:
        ...
        if process.extractOne(t['customer_text'].casefold(), texts, scorer=fuzz.ratio, score_cutoff=85):
            continue
        chosen.append({'id': f"c3_{len(chosen)+1:03d}", 'thread_id': t['thread_id'], 'text': t['customer_text'], 'split': 'confirmation'})
    ...
    write_json(out / 'SAMPLE.lock.json', {'n': 60, 'seed': 2026091703, 'purpose': 'one-shot confirmation; AI labels before inference, never hand-labelled claim',
               ...
               'sources': {str(f.relative_to(Paths.ROOT)).replace('\\', '/'): sha256(f)
                           for f in [*sources, Paths.THREADS, Paths.ROOT / 'scripts/20_lock_confirmation.py']}})
    print('Locked 60 unseen messages; label independently before inference')
