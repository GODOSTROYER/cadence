...
def main(argv=None):
    ...
    excluded_tweets = {str(turn["tweet_id"]) for t in threads if t["thread_id"] in excluded_ids
    ...
    for t in pool:
        ...
        if turns & selected_tweets or process.extractOne(text.casefold(), selected_texts, scorer=fuzz.ratio, score_cutoff=85):
            continue
        chosen.append({"id": f"h_{len(chosen)+1:03d}", "thread_id": t["thread_id"], "text": text,
                       "created_at": t.get("created_at"), "split": "test"})
    ...
    with (args.out / "human_labels.csv").open("w", newline="", encoding="utf-8") as f:
        fields = ["id", "text", "intent", "secondary_intent", "should_escalate", "escalation_reason_code", "sentiment", "notes", "annotator", "label_source"]
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        writer.writerows({"id": r["id"], "text": r["text"]} for r in chosen)
    write_json(args.out / "HOLDOUT.lock.json", {
        "version": 1, "seed": 2026, "n": len(chosen), "eligible_pool": len(pool),
        ...
        "sources": {str(path.relative_to(Paths.ROOT)).replace('\\', '/'): sha256(path) for path in [*sources, Paths.THREADS]},
    })
    print(f"Locked {len(chosen)} examples from {len(pool)} eligible threads in {args.out}; labels remain blank.")
    return 0
