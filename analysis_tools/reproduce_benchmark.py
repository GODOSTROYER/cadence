...
def summarize(golden, rows, n_boot=2000):
    ...
    return {
        ...
        'intent_detail': intent_metrics(truth, pred, intent_ids()),
...
def judge_summary(scores, ids):
    ...
    if len(indexed) != len(scores) or len(systems) != 2:
...
def main():
    ...
    parser.add_argument('--directory', type=Path, default=Paths.RESULTS / 'holdout')
