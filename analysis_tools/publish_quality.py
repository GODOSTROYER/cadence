"""Publish an auditable acceptance summary from the final measured evidence."""
...
def report():
    rows = []
    for name, title in [
        ("quality_dev_v3", "Development (30)"),
        ("quality_confirmation", "Confirmation (60)"),
    ]:
        root = Paths.RESULTS / name
        summary = read_json(root / "summary.json")
        accept = read_json(root / "ACCEPTANCE.json")
        reviewed = read_json(root / "astra_coverage.json")
        for system, side in [("agent", "right"), ("quality", "left")]:
            m = summary["comparisons"]["quality"][side]
            runtime = summary["runtime"][system]
            counts = accept["counts"][system]
            rows.append(
                f"| {title} | {system} | {100 * m['escalation_recall']:.1f}% | {counts['missed_escalations']} | {counts['automatic']}/{summary['n']} | {reviewed['systems'][system]['useful_automatic']}/{summary['n']} | {runtime['prompt_tokens'] + runtime['output_tokens']:,} | {runtime['p95_ms'] / 1000:.2f}s |"
            )
    acceptance = read_json(Paths.RESULTS / "quality_confirmation/ACCEPTANCE.json")
    gates = "\n".join(f"- **{'PASS' if v else 'NOT MET'}**: `{k}`" for k, v in acceptance["gates"].items())
    return f"""# Quality repairs, confirmation and acceptance

## Outcome

All implementation, AI review and fresh-confirmation work is complete. **{acceptance["decision"]}** Completing an evaluation does not require promoting a candidate that fails its gates. The hosted demo continues with the existing reference agent. Form submission is intentionally untouched.
...
def main():
    ...
    args = p.parse_args()
    path = Paths.ROOT / "docs/QUALITY_ACCEPTANCE.md"
    content = report()
    if args.check:
        if path.read_text(encoding="utf-8") != content:
            raise ValueError("Acceptance documentation differs from evidence")
    else:
        path.write_text(content, encoding="utf-8")
    print(
        "Verified quality acceptance documentation"
        if args.check
        else "Published quality acceptance documentation"
    )
